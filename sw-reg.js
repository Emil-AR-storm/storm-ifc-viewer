...
// domene, uten proxy, og har ingen bruk for offline. Registrerte vi der også,
// ville vi hatt to service workere å tenke på i stedet for én.
//
// ---------------------------------------------------------------------------
...
function visNyVersjon(ventende) {
  const el = document.getElementById("nyVersjon");
  if (!el) return;
  el.textContent = "";
  const tekst = document.createElement("span");
  tekst.textContent = t("Ny versjon klar.") + " ";
  const knapp = document.createElement("button");
  knapp.type = "button";
  knapp.textContent = t("Last inn på nytt");
  knapp.onclick = () => {
    // Bytte skjer i to trinn: vi ber den ventende ta over, og laster først
    // siden på nytt når nettleseren melder at den FAKTISK har tatt over.
    // Lastet vi med en gang, ville vi like gjerne fått den gamle igjen.
    ventende.postMessage({ type: "hopp-over-venting" });
    knapp.disabled = true;
  };
  ...
  addEventListener("load", () => {
    navigator.serviceWorker.register("/sw.js", { scope: "/", updateViaCache: "none" })
      .then((reg) => {
        ...
        if (reg.waiting && navigator.serviceWorker.controller) visNyVersjon(reg.waiting);
        ...
        reg.addEventListener("updatefound", () => {
          ...
          ny.addEventListener("statechange", () => {
            ...
            if (ny.state === "installed" && navigator.serviceWorker.controller) visNyVersjon(ny);
          });
        });
      })
      .catch(() => {
    ...
    // Hvilken versjon kjører telefonen? Svaret legges på window slik at det
    // kan leses i ⚙ Innstillinger, og — viktigere — slik at du kan spørre
    // montøren «hva står det der» i stedet for å gjette.
